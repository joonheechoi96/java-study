...
public class StreamPipeLineExample {
	...
	public static void main(String[] args) {
		// 불변 리스트를 만드는 2가지 방법
		// 1. Arrays.asList(크기 불변 리스트): 요소 변경 가능(set), add/remove 불가능 + null 허용
...
//		);
		...
		// 2. List.of (완전 불변 리스트): 요소+크기 변경 불가, add/remove 불가능, null 금지
...
//		studentStream.mapToInt(Student::getScore); // ??
		...
		// Quiz : 학생 객체를 학생의 이름으로 변환해서(중간처리) 출력해보기(최종처리)
		// 학생 객체를 String 으로 매핑하면 됨.
		// 힌트 : map() , forEach()
