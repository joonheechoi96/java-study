...
//		findClass();
		try {
			...
			System.out.println("클래스가 존재하지 않습니다 ㅋ");
			
			// 다양한 예외 출력 방법
			System.out.println(e.getMessage()); // 예외 메세지 출력 (예외 객체를 만들 때 매개값으로 사용한 메세지)
			System.out.println(e.toString());  // 예외 클래스 이름과 메세지를 함께 출력
	...
	// 주로 서비스 계층에서 예외를 처리하지 않고 컨트롤러나 UI 계층에서 처리하도록 넘길 떄
	// (예: 로그남기기,재시도, 사용자에게 알림, 특정 페이지로 이동 등)
