...
//		List list = new ArrayList(); // 제네릭을 생략하면 Object 로 간주 (권장 안함)
//		List<String> list = new ArrayList<String>(); // 
		List<String> list = new ArrayList<>(); // 생략하면 왼쪽 List에 지정된 타입을 따라 감(타입 추론)
		
		// 객체 추가
//		list.add(true); // 제네릭 타입에 명시된 타입의 데이터만 추가 가능
		list.add("Java"); // 비어있는 마지막 인덱스에 객체를 저장 (0부터)
		list.add("JDBC");
		list.add("Servlet/JSP");
		list.add(2, "Datebase"); // 2번 index에 저장 (하나씩 밀려남)
		list.add("iBATIS");
//		list.add(7, "TEST"); // 예외 발생
		...
		// (참고만) forEach(익명 구현 객체 또는 람다식);
		// 함수형 프로그래밍이란 "함수를 값처럼 사용" 하는 프로그래밍 방식
		list.forEach(item -> System.out.println(item)); // 콜백함수
		System.out.println();
		
		// 객체 검색
		if (!list.isEmpty()) { // 컬렉션이 비어있는지 조사
			System.out.println("JDBC 있니? " + list.contains("JDBC"));
			System.out.println("JDBC 어디에 있니? " + list.indexOf("JDBC"));
			System.out.println("Database 있니? " + list.contains("Database"));
			System.out.println("Database 어디에 있니? " + list.indexOf("Database"));
		}
		System.out.println();
